...
wss.on("connection", function connection(ws) {
	...
	ws.on("message", function message(data: any) {
		...
		if (message.type === "sender") {
			...
			if (senderSocket) {
				console.log(
					"A sender is already connected, disconnecting previous sender"
				);
			...
			if (receiverSocket) {
				console.log(
					"A receiver is already connected, disconnecting previous receiver"
				);
			...
			receiverSocket?.send(
				JSON.stringify({ type: "create-offer", offer: message.sdp })
			);
			console.log("///////////", message.sdp);
			console.log("Offer sent to receiver");
		} else if (message.type === "create-answer") {
			senderSocket?.send(
				JSON.stringify({ type: "create-answer", answer: message.sdp })
			);
